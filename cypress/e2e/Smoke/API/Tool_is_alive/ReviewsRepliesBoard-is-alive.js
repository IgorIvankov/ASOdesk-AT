...
describe('Reviews & Replies Board should be alive and main requests should response 200', function () {
    ...
    it('app-reviews should response 200 and not be empty ', function () {
        cy.request({
            method: 'GET',
            followRedirect: true, log: true, //turn off
            url: '/api/' + app + '/app-reviews?rating=5&languages=es&start=' + monthAgo + '&end=' + toDaysDate + '&order=newest&featured=false&tag_search_type=any&countries=' + [constant.GpFavCountryIds[4].toUpperCase()] + '&without_tags=false',
            headers: {
                'accept': 'application/json',
                'Authorization': auth.token,
            },
